...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let result = Ticket::new(title.clone(), description.clone(), status.clone());

    let err_msg = result.unwrap_err();

    if !err_msg.is_empty() {
 if err_msg == "Title cannot be empty" || err_msg == "Title cannot be longer than 50 bytes" {
        panic!("{}", err_msg)
    } else {
        return Ticket::new(title, String::from("Description not provided"), status).unwrap();
    }
    }

   

    Ticket { title: (), description: (), status: () }.unwrap()
