...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    if let Err(msg) = Ticket::new(title.clone(), description.clone(), status.clone()) {
    ...
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(msg ) => {
            if msg == "Title cannot be empty" || msg == "Title cannot be longer than 50 bytes" {
                ...
                Ticket::new(title, description:String::from("Description not provided", status).unwrap();
            }
        },
    }
}
