...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() {
            return Err(TicketNewError::TITLE_ERROR("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::TITLE_ERROR("Description cannot be longer than 500 bytes".to_string()));
