...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TITLE_ERROR(msg)) => panic!("{}", msg),
        Err(TicketNewError::DESCRIPTION_ERROR(msg)) => Ticket::new(title, "Description not provided".to_string(), status).unwrap()
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TITLE_ERROR("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TicketNewError::TITLE_ERROR("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(TicketNewError::DESCRIPTION_ERROR("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(TicketNewError::DESCRIPTION_ERROR("Description cannot be longer than 500 bytes".to_string()));
        }
