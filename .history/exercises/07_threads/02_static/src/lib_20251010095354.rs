...
pub fn sum(slice: &'static [i32]) -> i32 {
    let right = slice[slice.len() / 2..].to_vec(); // right = slice.slice(slice.len() / 2);
    let thread1 = thread::spawn(move || slice.iter().sum::<i32>());
    let thread2 = thread::spawn(move || right.iter().sum::<i32>());
