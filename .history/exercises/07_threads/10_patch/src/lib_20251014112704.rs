...
impl TicketStoreClient {
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, OverloadedError> {
        ...
        self.sender
            ...
            .map_err(|_| OverloadedError)?; // Send đến server chính 
        Ok(response_receiver.recv().unwrap())
    }
    ...
    pub fn update(&self, ticket_patch: TicketPatch) -> Result<Option<Ticket>, OverloadedError> {
        ...
        self.sender
            ...
            .map_err(|_| OverloadedError)?; // Send đến server chính 
        Ok(response_receiver.recv().unwrap()) // Nhận Response từ server chính -> Trả kết quả cho client
    }
}
...
pub fn server(receiver: Receiver<Command>) { // Config server chính để nhận Command và bắn response
    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                let updated_ticket = ticket.map(|ticket| {
                    ...
                    ticket.clone() // clone để gửi ra ngoài
                });

                let _ = response_channel.send(updated_ticket);
            }
            Err(_) => {
