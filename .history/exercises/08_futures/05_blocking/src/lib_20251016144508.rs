...
pub async fn echo(listener: TcpListener) -> Result<(), anyhow::Error> {
    loop {
        ...
        let mut socket = socket.into_std()?; // chuyển sang std::net::TcpStream
        socket.set_nonblocking(false)?;      // cần blocking mode cho std::io

        // 👇 di chuyển sang threadpool blocking
        task::spawn_blocking(move || {
            let mut buffer = Vec::new();
            socket.read_to_end(&mut buffer)?;
            socket.write_all(&buffer)?;
            Ok::<_, std::io::Error>(()) // phải return Result để dùng ? bên ngoài
        })
        .await??; // await Future và Result lồng nhau
    }
}
